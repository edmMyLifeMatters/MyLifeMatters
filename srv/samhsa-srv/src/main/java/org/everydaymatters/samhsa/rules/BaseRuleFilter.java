...
abstract public class BaseRuleFilter
    implements Filter
{
    ...
    abstract protected void evaluate( HttpServletRequest req, DB db, String profileId, DBObject profile, 
        ...
        if ( methods.contains( method.toUpperCase() ) )
        {
            ...
            if ( profileId!=null )
            {
                ...
                try
                {
                    ...
                    Responder responder = (Responder) application.getAttribute( ApplicationContext.ATTR_RESPONDER );
                    PushNotifier notifier = ApplicationContext.getNotifier( application );
                    
                    evaluate( hreq, db, profileId, profile, body, bdo, responder, notifier );
                }
                catch ( Exception e )
                {
